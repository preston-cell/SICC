...
function calculateScore(parsed, analysisResult) {
    ...
    const hasWill = parsed.existingDocs?.hasWill;
    const hasTrust = parsed.existingDocs?.hasTrust;
    const hasPOAFinancial = parsed.existingDocs?.hasPOAFinancial;
    const hasPOAHealthcare = parsed.existingDocs?.hasPOAHealthcare;
    const hasHealthcareDirective = parsed.existingDocs?.hasHealthcareDirective;
...
function buildComprehensivePrompt(parsed) {
...
async function callE2B(prompt, baseUrl, outputFile) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return {
                success: false,
                error: `E2B API call failed: ${response.status} ${errorText}`
            };
        }
                ...
                prioritizedRecommendations: [
                    {
                        ...
                        dependsOn: [],
                        detailedSteps: [
                            "Review the raw analysis output below",
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e9c7d1bf._.js.map