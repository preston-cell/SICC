...
async function createGo({
  ...
  const setGoEnv = async (goDir) => {
    ...
    if (isUnix && goDir === goGlobalCacheDir) {
      ...
      const tmpBase = await (0, import_build_utils2.getWriteableDirectory)();
      const tmpModCache = (0, import_path.join)(tmpBase, "pkg");
      const tmpBuildCache = (0, import_path.join)(tmpBase, ".cache");
      await Promise.all([
        (0, import_fs_extra.mkdirp)((0, import_path.join)(goDir, "pkg", "mod")),
        (0, import_fs_extra.mkdirp)((0, import_path.join)(goDir, ".cache", "go-build")),
        (0, import_fs_extra.symlink)((0, import_path.join)(goCacheDir, "pkg", "mod"), tmpModCache),
        (0, import_fs_extra.symlink)((0, import_path.join)(goCacheDir, ".cache", "go-build"), tmpBuildCache)
      ]);
...
function getRenamedEntrypoint(entrypoint) {
  ...
  if (filename.startsWith("[")) {
    ...
    (0, import_build_utils3.debug)(`Renamed entrypoint from ${entrypoint} to ${newEntrypoint}`);
...
async function build({
  ...
  const goPath = await (0, import_build_utils3.getWriteableDirectory)();
  const srcPath = (0, import_path2.join)(goPath, "src", "lambda");
  const downloadPath = meta.skipDownload ? workPath : srcPath;
  await (0, import_build_utils3.download)(files, downloadPath, meta);
  const undo = {
    fileActions: [],
    directoryCreation: [],
    functionRenames: []
  ...
  try {
    if (env.GIT_CREDENTIALS) {
      (0, import_build_utils3.debug)("Initialize Git credentials...");
  ...
  `);
    ...
    if (config && config.includeFiles) {
      ...
      for (const pattern of patterns) {
        const fsFiles = await (0, import_build_utils3.glob)(pattern, entrypointDirname);
    ...
    const runtime = await (0, import_build_utils3.getProvidedRuntime)();
    const lambda = new import_build_utils3.Lambda({
      files: { ...await (0, import_build_utils3.glob)("**", outDir), ...includedFiles },
    ...
    (0, import_build_utils3.debug)(`Go Builder Error: ${error}`);
    throw error;
  } finally {
    try {
      ...
      (0, import_build_utils3.debug)("Cleanup Error: " + error);
    }
  }
}
async function buildHandlerWithGoMod({
  ...
  if (goModPath && isGoModInRootDir) {
    (0, import_build_utils3.debug)(`[mod-root] Write main file to ${downloadPath}`);
    mainGoFile = (0, import_path2.join)(downloadPath, MAIN_GO_FILENAME);
  } else if (goModDirname && !isGoModInRootDir) {
    (0, import_build_utils3.debug)(`[mod-other] Write main file to ${goModDirname}`);
    mainGoFile = (0, import_path2.join)(goModDirname, MAIN_GO_FILENAME);
  } else {
    (0, import_build_utils3.debug)(`[entrypoint] Write main file to ${entrypointDirname}`);
  ...
  (0, import_build_utils3.debug)("Tidy `go.mod` file...");
  try {
    await go.mod();
  } catch (err) {
  ...
  (0, import_build_utils3.debug)("Running `go build`...");
...
async function buildHandlerAsPackageMain({
  ...
  (0, import_build_utils3.debug)('Building Go handler as package "main" (legacy)');
  ...
  (0, import_build_utils3.debug)("Running `go get`...");
  try {
    await go.get();
  } catch (err) {
...
function getNewHandlerFunctionName(originalFunctionName, entrypoint) {
  ...
  (0, import_build_utils3.debug)(`Found exported function "${originalFunctionName}" in "${entrypoint}"`);
...
`;
  ...
  (0, import_build_utils3.debug)(`Writing ${destGoModPath}`);
...
async function startDevServer(opts) {
  ...
  (0, import_build_utils3.debug)(`SPAWNING ${executable} CWD=${tmp}`);
...
async function prepareCache({
  ...
  if (stat.isSymbolicLink()) {
    ...
    (0, import_build_utils3.debug)(`Preparing cache by moving ${goGlobalCacheDir} -> ${goCacheDir}`);
    await (0, import_fs_extra2.unlink)(goCacheDir);
    await (0, import_fs_extra2.move)(goGlobalCacheDir, goCacheDir);
  }
  const cache = await (0, import_build_utils3.glob)(`${localCacheDir}/**`, workPath);
  return cache;
}
