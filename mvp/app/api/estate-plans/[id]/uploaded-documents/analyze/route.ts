...
// POST /api/estate-plans/[id]/uploaded-documents/analyze - Analyze an uploaded document
export async function POST(
  ...
  try {
    ...
    // Extract text from PDF using Claude's document capability
...
Format the output as plain text with clear section breaks. Do not summarize - extract the complete text.`,
      ...
      await prisma.uploadedDocument.update({
        ...
        data: {
          analysisStatus: 'failed',
          analysisError: 'Failed to extract text from PDF',
        },
      })
      return NextResponse.json(
    ...
    // Save extracted text and update status to analyzing
    await prisma.uploadedDocument.update({
      where: { id: documentId },
      data: {
        extractedText,
    ...
    const beneficiaryDesignations = await prisma.beneficiaryDesignation.findMany(
      {
        where: { estatePlanId },
      }
    )
    ...
    intakeData.forEach((section: { data: string; section: string }) => {
      try {
    ...
    // Run AI analysis
    const analysisPrompt = `You are an expert estate planning attorney. Analyze the following legal document and provide a comprehensive analysis.
...
${JSON.stringify(beneficiaryDesignations, null, 2)}

State of Residence: ${estatePlan?.stateOfResidence || 'Unknown'}
...
Respond ONLY with the JSON object, no additional text.`
    ...
    try {
      ...
      const analysisTextContent = analysisMessage.content.find(
        (block) => block.type === 'text'
      )
      const responseText = analysisTextContent?.text || ''
        ...
        potentialIssues: [
          {
            ...
            details:
              'The document could not be fully analyzed. Please try again or contact support.',
            severity: 'warning',
            recommendation:
              "Try re-uploading the document or ensure it's a valid PDF.",
          },
        ],
        hypotheticals: [],
        recommendations: [],
        plainEnglishSummary: 'Analysis could not be completed.',
      }
    }
    ...
    // Try to update the document status to failed if we have the documentId
    try {
